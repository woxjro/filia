...
public:
  ...
  mlir::michelson::Builtin builtin;
  // Name of builtin or module
...
public:
  ...
  static
  ValueDomain make_builtin(mlir::michelson::Builtin builtin) {
  ...
  mlir::Value getValue(mlir::OpBuilder& builder,
    ...
    case BUILTIN:
      {
        auto b = builder.create<mlir::michelson::BuiltinOp>(location, builtin);
        return b.result();
      }
    case MODULE:
      {
        auto b = builder.create<mlir::michelson::ModuleOp>(location, name);
        return b.result();
      }
    case ARGUMENT:
      {
...
public:
   ...
   */
  void cellAlloc(mlir::michelson::CellAllocOp op) {
    auto d
      = op.initial()
   ...
   * @param op allocation
   */
  void cellStore(mlir::michelson::CellStoreOp op) {
  ...
  mlir::Value cellLoad(mlir::michelson::CellLoadOp op, const std::vector<mlir::Value> &argValues) {
    ...
    if (cd.getStatus() == CellDomain::VALUE) {
      ...
      auto b = builder.create<mlir::michelson::MkReturnOp>(op.getLoc(), v);
      return b.result();
    } else {
      return mlir::Value();
    }
