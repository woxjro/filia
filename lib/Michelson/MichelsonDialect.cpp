...
namespace detail {
    struct ListTypeStorage : public mlir::TypeStorage {
      ...
      ListTypeStorage(llvm::ArrayRef<mlir::Type> elementType)
      ...
      /// itself, see the `ListType::get` method further below.
      /// Note: This method isn't necessary because KeyTy can be directly
      /// constructed with the given parameters.
      static KeyTy getKey(llvm::ArrayRef<mlir::Type> elementType) {
      ...
      static ListTypeStorage *construct(mlir::TypeStorageAllocator &allocator,
        ...
        llvm::ArrayRef<mlir::Type> elementType = allocator.copyInto(key);
      ...
      llvm::ArrayRef<mlir::Type> elementType;
    };

    struct PairTypeStorage : public mlir::TypeStorage {
      /// The `KeyTy` is a required type that provides an interface for the storage
      /// instance. This type will be used when uniquing an instance of the type
      /// storage. For our struct type, we will unique each instance structurally on
      /// the elements that it contains.
      ...
      /// A constructor for the type storage instance.
      PairTypeStorage(llvm::ArrayRef<mlir::Type> elementTypes)
      ...
      /// Define a construction function for the key type from a set of parameters.
      /// These parameters will be provided when constructing the storage instance
      /// itself, see the `PairType::get` method further below.
      /// Note: This method isn't necessary because KeyTy can be directly
      /// constructed with the given parameters.
      ...
      /// `KeyTy`. The given allocator must be used for *all* necessary dynamic
      /// allocations used to create the type storage and its internal.
      static PairTypeStorage *construct(mlir::TypeStorageAllocator &allocator,
                                          const KeyTy &key) {
        // Copy the elements from the provided `KeyTy` into the allocator.
        llvm::ArrayRef<mlir::Type> elementTypes = allocator.copyInto(key);

        // Allocate the storage instance and construct it.
        return new (allocator.allocate<PairTypeStorage>())
            PairTypeStorage(elementTypes);
      }
