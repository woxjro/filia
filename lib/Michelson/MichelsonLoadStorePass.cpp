...
#include "Michelson/MichelsonLoadStorePass.h"
...
public:
  BlockInvariantFixpointQueue(mlir::Block* entry) {
    ...
    auto cellTypeId = mlir::michelson::CellType::getTypeID();
...
template<>
void addOpArgs<mlir::michelson::RetBranchOp>(
        BlockInvariantFixpointQueue& inv,
        std::vector<mlir::Operation*>& toDelete,
        const std::vector<mlir::Value>& argValues,
        const LocalsDomain& term,
        mlir::Operation* opPtr) {
  assert(mlir::isa<mlir::michelson::RetBranchOp>(opPtr));

  auto op = mlir::cast<mlir::michelson::RetBranchOp>(opPtr);
  auto builder(mlir::OpBuilder::atBlockEnd(opPtr->getBlock()));
  ...
  builder.create<mlir::michelson::RetBranchOp>(op.getLoc(), op.value(),
...
TermSubstFnMap mkTermMap(void) {
  ...
  addOp<mlir::michelson::RetBranchOp>(termFns);
  return termFns;
}
...
void applyBlockOps(LocalsDomain& locals, mlir::Block* block) {
  for (auto opPtr = block->begin(); opPtr != block->end(); ++opPtr) {
    if (auto derivedOp = mlir::dyn_cast<mlir::michelson::CellAllocOp>(opPtr)) {
      locals.cellAlloc(derivedOp);
    } else if (auto derivedOp = mlir::dyn_cast<mlir::michelson::CellStoreOp>(opPtr)) {
      locals.cellStore(derivedOp);
    } else if (auto op = mlir::dyn_cast<mlir::michelson::CellLoadOp>(opPtr)) {
      // Do nothing
    }
  }
}
...
void optimizeFunction(mlir::MLIRContext* ctx,
  ...
  auto michelsonValueType = mlir::michelson::ValueType::get(ctx);
  ...
  for (auto& p : inv.blockMap) {
    ...
    for (auto p : argInfo.argVec) {
      if (p.first) {
        auto arg = block->addArgument(michelsonValueType, builder.getUnknownLoc());
...
class MichelsonLoadStoreOptimization
   : public PassWrapper<MichelsonLoadStoreOptimization, OperationPass<mlir::ModuleOp>> {
public:

  llvm::StringRef getArgument() const final {
    // This is the argument used to refer to the pass in
    // the textual format (on the commandline for example).
    return "michelson-load-store";
  }

  void runOnOperation() override;
};

void MichelsonLoadStoreOptimization::runOnOperation() {
...
void registerLoadStorePass() {
  mlir::PassRegistration<mlir::michelson::MichelsonLoadStoreOptimization>();
}
