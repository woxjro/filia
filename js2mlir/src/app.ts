...
class BlockContext {
  ...
  private appendStatement(op: Op) {
    if (!this.statements) throw new Error('Block already complete.');
    if (op.isTerminal()) throw new Error('Internal statement is terminal.');
  ...
  declVariable(kind: mlirjs.VarKind, name: string, value: Value | undefined) {
    ...
    this.appendStatement(
      new mlirjs.LocalDecl(r, this.locals, kind, name, value),
    );
    this.locals = r;
  }
  ...
  endBlock(term: mlir.TerminalOp) {
    if (!this.statements) throw new Error('Block already ended');
  ...
  condBranch(cond: Value, trueId: BlockId, falseId: BlockId) {
    ...
    this.endBlock(
      new cf.CondBranchOp(
        cond,
        { id: trueId, args: args },
        { id: falseId, args: args },
      ),
    );
  }

  return(val: Value | null) {
    const v = val ? [{ value: val, type: mlirjs.ValueType }] : [];
...
class Scope {
  ...
  constructor(parent: Scope | null, config: ScopeConfig) {
    ...
    if (config.newModule) {
      ...
      if (!parent) throw new Error('Expected parent when newModule not set');
  ...
  translateFunctionExpression(e: estree.FunctionExpression) {
    ...
    if (e.generator) {
      throw new TranslationError(
        e.loc,
        'Generator functions are not yet supported.',
      );
    }
    if (e.async) {
      throw new TranslationError(
  ...
  */
  ...
  translateAssignmentExpression(e: estree.AssignmentExpression): Value {
    ...
    switch (e.operator) {
      ...
      case '&=':
        this.addWarning(
          e.loc,
          `Unsupported assignment operator ${e.operator}.`,
        );
        break;
      default:
        return assertNever(e.operator);
    ...
    switch (left.type) {
      ...
      default:
        this.addWarning(
          e.loc,
          `Unsupported assignment left-hand-side ${left.type}.`,
        );
        break;
    }
    return v;
  ...
  private translateConditionalExpression(
    e: estree.ConditionalExpression,
  ): Value {
  ...
  */
    ...
    for (const decl of d.declarations) {
      ...
      switch (id.type) {
        ...
        case 'MemberExpression':
          this.addWarning(
            id.loc,
            `Variable declaration ${id.type} unsupported`,
          );
          break;
      }
    }
  ...
  translateClassDeclaration(d: estree.ClassDeclaration) {
    var superClass: Value | null = d.superClass
        ...
        case 'MethodDefinition':
          switch (b.kind) {
            ...
            case 'method':
              {
                ...
                if (b.key.type !== 'Identifier') {
                  throw new TranslationError(
                    b.loc,
                    `Expected method name to be identifier`,
                  );
                }
                const methodRef = this.module.freshFunctionId(
                  `${ctor.name}_${b.key.name}`,
                );
            ...
            case 'get':
              throw new TranslationError(
                b.loc,
                `get unsupported ${JSON.stringify(b.key)}`,
              );
            case 'set':
              throw new TranslationError(
                b.loc,
                `set unsupported ${JSON.stringify(b.key)}`,
              );
            default:
              assertNever(b.kind);
          }
  ...
  checkTopLevel(
    loc: estree.SourceLocation | undefined | null,
    msg: string,
  ): any {
    if (this.parent != null) throw new TranslationError(loc, msg);
  }
  ...
  */
   ...
   */
  translateStatement(
    s: estree.Directive | estree.Statement | estree.ModuleDeclaration,
  ): boolean {
    switch (s.type) {
...
if (process.argv.length != 5) {
  process.stderr.write(
    'Please provide (script|module) javascript_filename mlir_path.\n',
  );
  process.exit(-1);
}
// Get filetype
